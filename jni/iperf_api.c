...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    if (test->logfile) {//modified 2/23
	test->outfile = fopen(test->logfile, "a+");
    //test->outfile = fopen("/cache/log.dat", "a+");
...
iperf_new_stream(struct iperf_test *test, int s)
{
    ...
    /*doSS
    {
    	(&sp->result->interval_results)->tqh_first = NULL;
    	(&sp->result->interval_results)->tqh_last = &(&sp->result->interval_results)->tqh_first;
    } while (0);*/
